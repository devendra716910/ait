...
class User(db.Model):
    ...
    def __repr__(self):
        return f'User("{self.id}","{self.fname}","{self.lname}","{self.email}","{self.course}","{self.username}","{self.status}")'
...
# Create MeetLink Class with class type
class MeetLink(db.Model):
    id = db.Column(db.Integer, primary_key=True)
    link = db.Column(db.String(500), nullable=False)
    class_type = db.Column(db.String(50), nullable=False)  # 'sce', 'asce', 'nce', 'awd'

    def __repr__(self):
        return f'MeetLink("{self.id}", "{self.link}", "{self.class_type}")'

# Create the MeetLink table
...
# latter will check the condition
    admin=Admin(username='devendra',password=bcrypt.generate_password_hash('devendra123',10))
...
def userSignup():
    ...
    if request.method=='POST':
        ...
        if fname =="" or lname=="" or email=="" or password=="" or username=="" or course=="":
            flash('Please fill all the field','danger')
            ...
            if is_email:
                ...
                user=User(fname=fname,lname=lname,email=email,password=hash_password,course=course,username=username)
                db.session.add(user)
                db.session.commit()
...
def userUpdateProfile():
    ...
    if request.method == 'POST':
        ...
        username=request.form.get('username')
        course=request.form.get('course')
        if fname =="" or lname=="" or email=="" or username=="" or course=="":
            flash('Please fill all the field','danger')
            return redirect('/user/update-profile')
        else:
            session['username']=None
            User.query.filter_by(id=id).update(dict(fname=fname,lname=lname,email=email,course=course,username=username))
            db.session.commit()
...
@app.route('/admin/add-meet-link', methods=['POST', 'GET'])
def add_meet_link():
    if not session.get('admin_id'):
        return redirect('/admin/')
    ...
    if request.method == 'POST':
        ...
        if link and class_type:
            new_link = MeetLink(link=link, class_type=class_type)
            db.session.add(new_link)
            db.session.commit()
            flash('Google Meet Link added successfully!', 'success')
            return redirect('/admin/add-meet-link')
        else:
...
@app.route('/user/meet-links/<string:class_type>')
def user_meet_links_by_type(class_type):
    if not session.get('user_id'):
        return redirect('/user/')
    
    # Fetch links for the specified class type
    meet_links = MeetLink.query.filter_by(class_type=class_type).all()
    return render_template('user/gl.html', title=f'Google Meet Links for {class_type.upper()}', meet_links=meet_links, class_type=class_type)
